...
const ALLOWED_QUALITIES = ["4K", "2160", "1080", "720", "AUTO"];
...
function isValidQuality(quality) {
  ...
  return ALLOWED_QUALITIES.some(q => 
    q.includes(normalized) || normalized.includes(q) || 
    normalized === "2160p" || normalized === "1080p" || normalized === "720p" ||
    normalized === "UHD" || normalized === "FHD" || normalized === "HD"
  );
}
function formatStreamsForNuvio(decryptedData, serverName, mediaDetails) {
  try {
    ...
    const formattedSubtitles = (data.subtitles || [])
      .filter(sub => !sub.url || sub.url.startsWith("https://"))
      .map((sub) => ({
        url: sub.url,
